...
export function OnboardingFlow({
  ...
  async function confirm() {
    ...
    try {
      ...
      if (result.seeded || result.reason === "existing-plan") onSeeded();
    } catch (err) {
  ...
  return (
    <Card>
      ...
      {themeId && (
        <p className="mt-3 text-sm text-forest-light">{ONBOARDING_THEMES.find((theme) => theme.id === themeId)?.detail}</p>
      )}
      {!catalogReady && (
