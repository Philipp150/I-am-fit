...
## 17. Nachtrag: Authored Posen korrigiert, Pose-Picker nutzbar gemacht

Nachtrag 16 hat die Winkelkonvention für die **aus dem Video abgeleitete Spur** korrigiert (`pose-map.ts`: SVG `rotate()` dreht im Uhrzeigersinn, weil y nach unten zeigt). Die **handgeschriebenen PoseId-Keyframes** (`poses.ts`) nutzten weiter die alte, entgegengesetzte Konvention – deshalb kippte „Nacken links“ nach rechts, gebeugte Ellbogen schlugen nach außen statt zum Körper, „Schritt rechts“ hatte verschränkte Beine, und bei liegenden Posen ragte der Kopf über den Rand der viewBox hinaus. Beide Quellen rechnen jetzt mit derselben Konvention, dokumentiert in `poses.ts` und `pose-map.ts`. Die Geometrie (Maße, Vorwärtskinematik, viewBox) liegt einmal in `pose-geometry.ts`; `StickFigure.tsx` zeichnet daraus, `poses.test.ts` misst dieselbe gezeichnete Figur statt nur die eingetippten Gradzahlen zu vergleichen. Seitenzwillinge (`neckLeft`/`neckRight`, `walkLeft`/`walkRight`, `tree`/`treeOther`, …) entstehen mit `mirrorPose`, sodass eine Seite nicht mehr unbemerkt von der anderen abweichen kann.

Die zweite Ursache für „absoluter Unfug“ (Philipp) war reine UX: Der Editor zeigte alle 53 Posen als unbeschriftete, fingernagelgroße Kacheln in einem 5-Spalten-Raster – für jeden Schritt erneut, ohne Gruppierung oder Suche. `PosePicker.tsx` (mit `pose-picker.ts`) ersetzt das: eine hervorgehobene Karte mit Name der gewählten Pose, dann wenige Vorschläge passend zum Schritttext (Wortabgleich auf Körperteile, Umlaut-tolerant), eine Suche, und erst dahinter – eingeklappt – alle Posen nach Körperregion gruppiert (Kopf/Nacken/Kiefer, Schultern/Arme, Rumpf/Drehung, Hüfte/Beine, Stand/Gehen, Atem, Sitzen/Liegen). Jede Kachel zeigt Name und Auswahlhäkchen.

Liegt an der Übung schon eine Bewegungsspur, macht ein Hinweisblock das für Nutzer sichtbar (statt die Pose-Auswahl einfach zu ignorieren): „Die Figur folgt der Bewegungsspur“ mit den Optionen „Spur verwenden“ und „Pose wählen“; „Pose wählen“ fragt nach und verlangt einen zweiten, expliziten Tap („Spur entfernen und Posen nutzen“), bevor die Spur wirklich verschwindet. Ohne diesen Tap bleibt die Spur die Quelle der Wahrheit für die Figur.

Sichtbarer Beweis liegt als Pose-Galerie vor (`scripts/pose-gallery.mjs` rendert alle 53 Posen mit Beschriftung) und als Playwright-Screenshots von Live-Seite (Zustand vorher) und lokalem Editor (Zustand nachher) im Handy-Viewport.
